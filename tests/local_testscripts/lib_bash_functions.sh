...
function run_pytest() {
  ...
  if ! python3 -m pytest "${project_root_dir}" "$@" --cov=pct_python_default_test; then
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : $HOME/venv/bin/pct_python_default_test --version"
  if ! "$HOME/venv/bin/pct_python_default_test" --version; then
